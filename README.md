...
- [ ] **Other targets (GPU)**. This may well require a new data structure (such as numpy v.s. torch/jax). An API-stable tensor structure that accepts various targets has probably yet existed in rust.
...
- [ ] **Advanced handling for mixed row-major and col-major**. Currently, all-row-major and all-col-major will not incur explicit memory clone; but for mixing cases, memory clone exists and could cost a lot of time (for transposition). There could be two ways to handle this problem: 1) option for warning or raise error when clone is required, optionally enforcing API user to handle matrix layout before calling BLAS; 2) explicitly handle mixed layout for f32/f64 type matrices.
